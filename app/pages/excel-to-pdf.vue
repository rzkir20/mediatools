      ...
      <div class="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center pt-8 pb-10 sm:pt-10 sm:pb-12 md:pt-12 md:pb-16">
        <div class="order-2 lg:order-1">
          <div class="mb-6 inline-flex items-center gap-3">
            ...
            <span class="uppercase tracking-[0.4em] text-xs font-heading font-black text-[#22C55E]">Excel</span>
          </div>
          ...
          <p class="text-white/50 text-base sm:text-lg max-w-lg mb-6 sm:mb-8">
            Konversi file Excel (XLSX) menjadi PDF dengan tampilan tabel yang
            rapi. Cocok untuk laporan keuangan, daftar hadir, dan data lainnya.
          </p>

          <div id="download-input" class="relative w-full max-w-2xl mb-8 sm:mb-10 space-y-4">
            <div class="bg-[#1A1A1A] p-4 sm:p-5 rounded-2xl border border-white/5 shadow-2xl shadow-black/40">
              <label class="block text-xs font-heading font-black uppercase tracking-widest text-white/50 mb-2">
                Pilih file XLSX
              </label>
              <label class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 cursor-pointer group">
                <div
                  ...
                  <p class="text-xs sm:text-sm text-white/70 truncate" v-if="file">
                    {{ file.name }}
                  </p>
                  <p v-else class="text-xs sm:text-sm text-white/40 flex items-center gap-2">
                    <iconify-icon icon="lucide:upload-cloud" class="text-base sm:text-lg" />
                    <span>Click to choose XLSX file</span>
                  </p>
                  <p class="mt-1 text-[10px] text-white/30 uppercase font-heading tracking-widest">
                    Maksimal 50 MB
                  </p>
                </div>
                ...
                <input type="file" class="hidden" @change="handleFileInput" accept=".xlsx" />
              </label>

              <div class="mt-4 grid grid-cols-1 sm:grid-cols-[1.3fr_auto] gap-3">
                <div>
                  <label class="block text-xs font-heading font-black uppercase tracking-widest text-white/50 mb-1">
                    Output
                  </label>
                ...
                <div class="flex items-end">
                  <button type="button"
                    ...
                    :disabled="!hasFile || converting" @click="onConvert">
            ...
            <p class="text-xs text-white/40">
              Didukung: hanya XLSX (Excel OOXML). Untuk file XLS lama, harap
              di-save ulang sebagai XLSX terlebih dahulu. Setiap sheet akan
              dirender sebagai tabel dalam PDF.
            </p>
          </div>
        </div>
          ...
          <div
            ...
            <div class="flex flex-col items-center gap-4 px-6">
              <div
                ...
                <iconify-icon icon="lucide:sigma" class="text-3xl text-[#22C55E]" />
              </div>
              <p class="font-heading text-lg sm:text-2xl font-black text-center leading-snug">
                Excel to PDF
              </p>
              <p class="text-xs text-white/50 text-center">
                Bagikan data Excel Anda sebagai PDF yang rapi dan mudah dibaca.
              </p>
            </div>
          </div>
        </div>
